...
MAX_RETRIES = 2
REQUIRED_FIELDS = ["live_with_family", "family_members", "parents_jobs"]
...
SYSTEM_PROMPT_EXTRACTION = (
    ...
    "Your only task is to extract exactly the required entity (YES/NO answer, FAMILY MEMBERS, PARENTS' JOBS) "
    "from the user's input without changing the conversation flow. "
    "Return only the clean extracted value (one word, short phrase), no greetings, no explanations, no extra words."
)
...
class FamilyModule:
    ...
    def extract_with_cohere(self, field_type, user_input):
        system_instruction = SYSTEM_PROMPT_EXTRACTION.replace("(YES/NO answer, FAMILY MEMBERS, PARENTS' JOBS)", field_type.upper())
    ...
    def run_conversation(self):
        # Step 1: Do you live with family?
        live_with_family = None
        family_prompt = "Do you live with your family?"
        while not live_with_family:
            live_with_family = self.ask_and_capture(family_prompt, "live_with_family", FIELD_RECORDING_DURATION["live_with_family"])

        # Step 2: Who is in your family?
        members = None
        members_prompt = "Who is in your family?"
        while not members:
            members = self.ask_and_capture(members_prompt, "family_members", FIELD_RECORDING_DURATION["family_members"])

        # Clean family members list
        member_list = [x.strip() for x in self.filled_fields['family_members'].replace('and', ',').split(',') if x.strip()]
        num_members = len(member_list)

        repeat_members = f"You said you have {num_members} people in your family: {', '.join(member_list)}."
        self.speak(repeat_members)
        print(f"\nCHATBOT: {repeat_members}")

        # Step 3: What do parents do?
        parents_jobs = None
        jobs_prompt = "What does your mother and father do?"
        while not parents_jobs:
            parents_jobs = self.ask_and_capture(jobs_prompt, "parents_jobs", FIELD_RECORDING_DURATION["parents_jobs"])

        # Confirm mother and father jobs properly
        mother_job = ""
        father_job = ""
        if "mother" in self.filled_fields["parents_jobs"].lower():
            try:
                mother_job = self.filled_fields["parents_jobs"].split("mother")[1].split(".")[0].strip()
            except:
                mother_job = ""
        if "father" in self.filled_fields["parents_jobs"].lower():
            try:
                father_job = self.filled_fields["parents_jobs"].split("father")[1].split(".")[0].strip()
            except:
                father_job = ""

        repeat_jobs = "Your"
        if mother_job:
            repeat_jobs += f" mother is {mother_job}"
        if father_job:
            if mother_job:
                repeat_jobs += " and"
            repeat_jobs += f" father is {father_job}"
        repeat_jobs += "."

        self.speak(repeat_jobs)
        print(f"\nCHATBOT: {repeat_jobs}")

        # Step 4: Build full expected summary
        self.expected_summary = (
            f"There are {num_members} people in my family: {', '.join(member_list)}."
        )
    ...
    def save_results(self):
        ...
        output_filename = f"family_output_{timestamp}.json"
