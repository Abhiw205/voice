# Refactored engine.py with LLM behavior dispatcher
...
class ModuleRunner:
    def __init__(self, config_path):
        ...
        self.field_results = {}  # NEW: store field results
        self.field_scores = {}  # NEW: store validation results
        ...
        while not value and attempts < max_attempts:
            ...
            if text:
                ...
                if field.get("validate_spell") and not self.is_valid_spelling(value):
                    self.speak("Hmm, that didn't sound like a spelling. Let's try again.")
                    value = ""
                    attempts += 1
                    continue

                if not value and field.get("retry_on_empty"):
                    self.speak("Hmm, I didn’t get that. Could you please try again?")
                    value = ""
                    attempts += 1
                    continue
    ...
    def handle_llm_behavior(self, field, user_input):
        ...
        if behavior == "extract_value":
            ...
            # ✨ Fallback catch for hallucinated LLM messages
            ...
            return self.generate_natural_acknowledgment(field["key"], user_input)

        elif behavior == "validate_svo":
            return self.validate_svo_pattern(user_input, field)
    ...
    def run(self):
            ...
            for field in self.fields:
                ...
                while attempts < max_attempts:
                    ...
                    if behavior and key in self.field_scores:
                        ...
                        attempts += 1
                        if attempts < max_attempts:
                            self.speak("Hmm, that wasn't quite right. Let's try again.")
                            print("\nCHATBOT: Hmm, that wasn't quite right. Let's try again.")
                    else:
                        break
        ...
        if self.config.get("validation_logic"):
            ...
            if total > 0:
                ...
                print("\nNo fields matched the validation criteria. Skipping validation logic.")
                self.filled_fields.update({
                    "score": "N/A",
                    "feedback": "No applicable validation fields."
                })
    
            # Ensure score is initialized to avoid UnboundLocalError
            if total == 0:
                score = 0
    ...
    # Step 3: Summary Section
        if self.config.get("summary_template") and all(
            ...
            self.speak("Debug mode on.")
            print("\nCHATBOT: Debug mode on.")
            self.filled_fields.update({
    ...
    def validate_debate_response(self, user_input: str, field: dict) -> bool:
        ...
        try:
            ...
            self.field_scores[field["key"]] = is_valid
            self.field_results[field["key"]] = "PASS" if is_valid else "FAIL"
            return is_valid
        except Exception as e:
